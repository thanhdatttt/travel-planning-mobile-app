...
export const signUp = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(201)
      .json(createResponse({ message: "Sign up successfully", data: user }));
  } catch (err: any) {
  ...
  try {
    ...
    if (!refreshToken) {
      return res
        .status(401)
        .json(
          createResponse({ message: "Unathorized", error: "Token is missing" }),
        );
    }
  ...
  try {
    ...
    if (!email || !newPassword) {
      return res
        .status(400)
        .json(
          createResponse({ message: "Bad request", error: "Missing fields" }),
        );
    }
    ...
    if (!user) {
      return res
        .status(404)
        .json(
          createResponse({ message: "Not found", error: "User not found" }),
        );
    }
    const localAuth = user.authProviders.find((p) => p.provider === "local");
    if (!localAuth) {
  ...
  try {
    ...
    if (record.expiresAt < new Date(Date.now())) {
      return res
        .status(403)
        .json(
          createResponse({ message: "Forbidden", error: "OTP is expired" }),
        );
    }
    ...
    return res
      .status(200)
      .json(createResponse({ message: "Email verified successfully" }));
  } catch (err: any) {
...
export const sendOTP = async (req: Request, res: Response) => {
  try {
    ...
    // reset pass otp
      ...
      if (!existingUser) {
        return res
          .status(404)
          .json(
            createResponse({ message: "Not found", error: "Email not found" }),
          );
      }
    ...
    return res
      .status(201)
      .json(createResponse({ message: "OTP sent to email" }));
  } catch (err: any) {
  ...
  try {
    ...
    if (!accessToken) {
      return res
        .status(400)
        .json(
          createResponse({ message: "Bad request", error: "Missing token" }),
        );
    }
